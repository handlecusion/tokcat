...
- **세션은 Swift를 컴파일할 수 없다**: 클라우드 VM은 Linux고 `native/`는 Swift + AppKit + SwiftUI를
  요구한다. 그래서 `ROUTINE_PROMPT.md` 4단계가 **CI를 세션의 컴파일러로** 쓰게 못 박는다 — 푸시 후
  체크런을 폴링하고, 빨간 잡 로그를 읽고 고쳐 다시 푸시(최대 3라운드), 그래도 빨갛다면 무엇이
  어떻게 빨간지 그대로 보고. `Swift build + test`가 ~2분이라 라운드당 비용은 그 정도다.
  자체 호스팅 macOS 러너는 이 한계를 바꾸지 않는다(세션 VM은 교체 불가) — 러너 소유자만 바뀐다.
  CI green은 "컴파일되고 테스트가 통과"까지다. 시각 검증은 사람이 Mac에서 한다(#90에서 실측).
